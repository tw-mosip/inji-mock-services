...
describe('ReviewPage Component', () => {
    ...
    test('confirm and submit button calls post_driver_details and shows success screen', async () => {
        relyingPartyService.post_driver_details.mockResolvedValue({
            status: 201,
        });
        render(_jsx(ReviewPage, {}));
    ...
    test('disables submit button while submitting', async () => {
        ...
        relyingPartyService.post_driver_details.mockReturnValue(promise);
        render(_jsx(ReviewPage, {}));
        const submitButton = screen.getByRole('button', { name: /confirm and submit/i });
        fireEvent.click(submitButton);
    ...
    test('logs error when submission fails', async () => {
        ...
        relyingPartyService.post_driver_details.mockRejectedValue(new Error('Network Error'));
        render(_jsx(ReviewPage, {}));
        const submitButton = screen.getByRole('button', { name: /confirm and submit/i });
        fireEvent.click(submitButton);
        await waitFor(() => {
