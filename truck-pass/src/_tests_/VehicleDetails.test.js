...
jest.mock('../../components/DropDownSelection', () => ({
    DropDownSelection: ({ setItemSelected, data, placeHolder }) => {
        return (_jsxs("select", { "data-testid": placeHolder, onChange: (e) => {
                const selected = data.find((d) => d.type === e.target.value || d.size === e.target.value);
                setItemSelected(selected);
            }, children: [_jsx("option", { children: placeHolder }), data.map((item) => (_jsx("option", { value: item.type || item.size, children: item.type || item.size }, item.id)))] }));
    },
}));
...
jest.mock('../../components/CertificateUploadSection', () => ({
    CertificateUploadingSection: ({ setFileUploaded, setDataInFile }) => {
        ...
        return _jsx("div", { "data-testid": "mock-upload-section", children: "Mock Upload" });
    },
}));
describe('VehicleDetails component', () => {
    const setup = () => render(_jsx(MemoryRouter, { children: _jsx(VehicleDetails, {}) }));
    beforeEach(() => {
        localStorage.clear();
