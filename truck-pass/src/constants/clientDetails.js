...
function checkEmptyNullValue(initialValue, defaultValue) {
    return initialValue && initialValue !== "" ? initialValue : defaultValue;
}
// Random string generator
function generateRandomString(strLength = 16) {
    let result = "";
...
const { SCOPE_USER_PROFILE, SCOPE_REGISTRATION, DISPLAY, PROMPT, GRANT_TYPE, MAX_AGE, CLAIMS_LOCALES, CLIENT_ID, ESIGNET_UI_BASE_URL, REDIRECT_URI, REDIRECT_URI_USER_PROFILE, REDIRECT_URI_REGISTRATION, ACRS, CLAIMS_USER_PROFILE, CLAIMS_REGISTRATION, } = window._env_;
const scopeUserProfile = checkEmptyNullValue(SCOPE_USER_PROFILE, "openid profile");
const scopeRegistration = checkEmptyNullValue(SCOPE_REGISTRATION, "openid profile");
...
const redirect_uri_userprofile = checkEmptyNullValue(REDIRECT_URI_USER_PROFILE, REDIRECT_URI);
const redirect_uri_registration = checkEmptyNullValue(REDIRECT_URI_REGISTRATION, REDIRECT_URI);
