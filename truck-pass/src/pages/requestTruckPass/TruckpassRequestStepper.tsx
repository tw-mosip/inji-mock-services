        ...
        return (
          <div key={id} className='flex'>
            <div key={id} className='flex flex-col items-center gap-y-2'>
              ...
              <p className={`${item.completed ? "text-[#079455]" : (item.inProgress ? "text-[#006DE7]" : "text-[#414651] font-[600]")}`}>{item.itemName}</p>
            </div>
            {/* <img src={stepsConnectingLine} className={`h-1.5 w-[0%] ${item.number === '6' && "hidden"}`} /> */}
          </div>
        )
      })
      }
